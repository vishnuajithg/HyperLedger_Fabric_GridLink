...
peer lifecycle chaincode package gridlink.tar.gz --path ${PWD}/../Chaincode/Gridlink --lang node --label gridlink_1.0
sleep 1
...
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.gridlink.com --channelID $CHANNEL_NAME --name Gridlink --version 1.0  --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
sleep 2
...
peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name Gridlink --version 1.0 --sequence 1  --tls --cafile $ORDERER_CA --output json

peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.gridlink.com --channelID $CHANNEL_NAME --name Gridlink --version 1.0 --sequence 1  --tls --cafile $ORDERER_CA --peerAddresses localhost:7051 --tlsRootCertFiles $PRODUCER_PEER_TLSROOTCERT --peerAddresses localhost:9051 --tlsRootCertFiles $CONSUMER_PEER_TLSROOTCERT --peerAddresses localhost:11051 --tlsRootCertFiles $UTILITYCOMPANY_PEER_TLSROOTCERT --peerAddresses localhost:12051 --tlsRootCertFiles $REGULATORYBODY_PEER_TLSROOTCERT
sleep 1
