...
class clientApplication {
    async submitTxn(
        ...
        try {
            ...
            if (txnType == "registerProducer") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            } 
            else if (txnType == "buyEnergyByUtil") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            } 
            else if (txnType == "produceEnergy") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            } 
            else if (txnType == "registerConsumer") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            } 
            else if (txnType == "handleEnergyRequest") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            } 
            else if (txnType == "registerUtility") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            } 
            else if (txnType == "requestEnergy") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            } 
            else if (txnType == "retrievePriceData") {
                resultBytes = await contract.evaluateTransaction(txnName, ...args);
            } 
            else if (txnType == "queryTxn") {
                resultBytes = await contract.evaluateTransaction(txnName, ...args);
            } else if (txnType == "storePriceData") {
                resultBytes = await contract.submit(txnName, {
                    arguments: [...args],
                    transientData: transientData,
                    endorsingOrganizations: ['ProducerMSP', 'UtilityCompanyMSP','RegulatoryBodyMSP'],
